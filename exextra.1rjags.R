### Extra Example 1: Newcomb Data --- Robust Analysis (outliers) ###
...
m <- jags.model("exextra.1model.bug", d, inits, n.chains=4, n.adapt=1000)
...
mean(apply(yrep, 1, max) >= max(newcomb))  # p.b for max
  ...
  # similar checks on the normal model indicate problems (try yourself)
